                ...
                function fetchQuestionsByCategory($conn, $table, $category_limits)
                {
                    ...
                    foreach ($category_limits as $category => $limit) {
                        ...
                        foreach ($types as $type) {
                            ...
                            $stmt = $conn->prepare("SELECT * FROM $table WHERE category = ? AND type = ? ORDER BY category ASC, type ASC, sub_type ASC LIMIT ?");
            ...
            <form action="../actions/submit_csc.php" method="post" id="quizForm">
                <?php foreach ($questions as $index => $q): ?>
                    ...
                    <div class="card" data-index="<?= $index ?>" data-id="<?= $q['id'] ?>">
                        <p> <?= htmlspecialchars($q['category']) ?> </p>
                        <p> <?= htmlspecialchars($q['type']) ?>/<?= htmlspecialchars($q['sub_type']) ?>/<?= htmlspecialchars($q['word']) ?> </p>
                        <p>Instruction: <?= htmlspecialchars($q['instruction']) ?></p>
